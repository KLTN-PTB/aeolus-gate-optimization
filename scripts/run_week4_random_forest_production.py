"""Full rolling Random Forest production run for Core Arrival.

The IO and fold assembly helpers are shared with the completed Linear run.
This script changes only the locked estimator factory and artifact namespace;
it does not alter the feature contract, target rows, folds, or preprocessing.
"""
...
from src.models.random_forest_models import (  # noqa: E402
    RANDOM_FOREST_BASELINE_CONFIG_PATH,
...
RUN_VERSION = "arrival_random_forest_rolling_run_v1"
...
class ResourceBlocked(RuntimeError):
    """Raised when a full RF fold cannot safely continue."""
...
def _rf_hashes(root: Path) -> dict[str, str]:
    hashes = _manifest_hashes(root)
    config_path = root / RANDOM_FOREST_BASELINE_CONFIG_PATH
    hashes["random_forest_baseline_config"] = _sha256(config_path)
...
def _linear_validation_fingerprints(root: Path, folds: tuple[RollingFold, ...]) -> dict[str, str]:
    path = root / "artifacts" / "manifests" / "arrival_linear_rolling_run_v1.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("status") != "PASS":
        raise RuntimeError("Linear production manifest is not PASS; RF row identity cannot be anchored")
    reports = payload.get("folds")
    if not isinstance(reports, list):
        raise RuntimeError("Linear production manifest lacks fold reports")
    ...
    for report in reports:
        ...
        fold_id = str(report.get("fold_id"))
        fingerprints[fold_id] = str(report["row_fingerprints"]["validation"])
    expected = {fold.fold_id for fold in folds}
    if set(fingerprints) != expected:
        raise RuntimeError("Linear validation fingerprints do not cover exactly the locked RF folds")
    return fingerprints
...
def build_preflight(root: Path) -> dict[str, Any]:
    ...
    linear_validation_fingerprints = _linear_validation_fingerprints(root, folds)
    capacity = _assert_runtime_capacity(root)
    hashes = _rf_hashes(root)
    ...
    return {
        ...
        "reference_linear_validation_fingerprints": linear_validation_fingerprints,
...
def _run(root: Path, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    project = load_base_config(project_root=root)
    folds = load_week4_rolling_folds(project_root=root)
    validation_fingerprints = _linear_validation_fingerprints(root, folds)
    spec = ExperimentSpec(
        ...
        expected_validation_row_fingerprints=validation_fingerprints,
    )
    ...
    return {
        ...
        "aggregate_development_metrics": {
            ...
            "rows": total_rows,
            "note": "pooled OOF diagnostics only; RF config remained locked",
        },
...
def _write_failure(root: Path, *, reason: str, preflight_path: Path, completed_folds: list[str]) -> None:
...
def main() -> int:
    ...
    try:
        ...
        _write_failure(
            ...
            completed_folds=error.completed_folds,
        )
        print(json.dumps({"status": "BLOCKED_RESOURCE", "reason": str(error)}, indent=2), file=sys.stderr)
        return 2
    except MemoryError as error:
        _write_failure(root, reason=f"MemoryError during unsampled full-data RF run: {error}", preflight_path=preflight_path, completed_folds=[])
        print(json.dumps({"status": "BLOCKED_RESOURCE", "reason": str(error)}, indent=2), file=sys.stderr)
        return 2
