...
from src.models.hist_gradient_boosting_models import (
    ...
    return pd.DataFrame(
        {
            ...
            "ARR_DELAY": [-10.0, 20.0],
            "flight_key": [f"hgb-{year}-a", f"hgb-{year}-b"],
...
def _fold_data() -> tuple[RollingFold, FoldData]:
    ...
    X.loc[X.index[0], "OP_CARRIER"] = "__UNKNOWN_FOR_HGB_TEST__"
...
def _run_once() -> tuple[object, pd.DataFrame]:
    config = load_hgb_baseline_config()
...
def test_locked_hgb_config_uses_train_weight_only_and_disables_internal_validation() -> None:
    config = load_hgb_baseline_config()
    spec = _spec()
    fold, data = _fold_data()
    weights = derive_balanced_class_weights(data.train.y_arr_cls)
    bundle = build_hgb_estimators(
    ...
    assert isinstance(bundle.regressor, HistGradientBoostingRegressor)
...
def test_hgb_fold_smoke_handles_tree_encoding_and_is_repeatable() -> None:
...
def test_hgb_factory_rejects_unlocked_seed_and_does_not_import_xgboost() -> None:
    config = load_hgb_baseline_config()
    fold, data = _fold_data()
    ...
    source = Path("src/models/hist_gradient_boosting_models.py").read_text(encoding="utf-8")
